...
export default function Hero({ heroContainerRef, expSectionRef }) {
  ...
  useGSAP(() => {
    gsap.fromTo(
      ...
      { y: 0, opacity: 1, rotateX: 0, duration: 1.2, stagger: 0.03, ease: "back.out(1.2)", scrollTrigger: { trigger: heroContainerRef.current, start: "top 80%", end: "top 30%", scrub: 0.5 } }
    );

    gsap.fromTo(
      ".subtitle",
      { y: 80, opacity: 0, scale: 0.9 },
      { y: 0, opacity: 1, scale: 1, duration: 1, delay: 0.5, ease: "power3.out", scrollTrigger: { trigger: heroContainerRef.current, start: "top 80%", end: "top 40%", scrub: 0.5 } }
    );

    gsap.to(heroContentRef.current, {
      opacity: 0, y: -50, duration: 1, ease: "power2.in",
      scrollTrigger: { trigger: heroContainerRef.current, start: "top 20%", end: "top -20%", scrub: 1 }
    });
  }, []);
  ...
  return (
    <Box ref={heroContainerRef} sx={{ height: "100vh", position: "relative", overflow: "hidden", background: "#000" }}>
      ...
      <Box ref={heroContentRef} sx={{ position: "absolute", bottom: "10%", left: 0, right: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", px: 3, zIndex: 2 }}>
        <Typography variant="h1" sx={{ color: "#fff", fontWeight: 800, overflow: "hidden", lineHeight: 1.2, mb: 2, fontSize: { xs: "3rem", sm: "4rem", md: "6rem", lg: "7rem" }, textTransform: "uppercase", letterSpacing: "0.02em", textShadow: "2px 2px 4px rgba(0,0,0,0.3)" }}>
